...
module.exports = function login (db, fingerprint, userloginID, loginType, app, space, details, cb) {
  ...
  person.byFingerprint(db, fingerprint, app, space, function (err, person) {
    if (err && err.no_account && err.person) return samePersonNewAccount(db, err.person, fingerprint, userloginID, loginType, app, space, details, cb)

    if (err) return cb(err)
    if (!person) return firstTimer(db, fingerprint, userloginID, loginType, app, space, details, cb)
